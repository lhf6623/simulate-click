...
| `模拟点击-<版本>.zip` | 压缩包，便于分发 |

## CI/CD

推送 `v*` 标签自动构建并发布 GitHub Release，也支持在 Actions 页面手动触发（`workflow_dispatch`）：
...
- **CGEvent** — 移动光标到目标位置并通过 HID 事件模拟点击
